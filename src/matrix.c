...
void matrix_copy(matrix in, matrix out) {
    ...
    if(in.values == NULL || out.values == NULL) {
        printf("Matrices shoulb not be null [copy]");
        exit(1);
    }
...
/* Adds matrix a with b and stores result in c */
...
/* Subtracts matrix a with b and stores result in c */
...
/* Multiply all elements of matrix in with given scalar */
void matrix_scalar_multiply(matrix in, double scalar, matrix out) {
    if(in.values == NULL) {
    ...
    for(int i = 0; i < in.row; i++) {
        for(int j = 0; j < in.col; j++)
            out.values[i][j] = in.values[i][j] * scalar;
    }
}
...
/* Fills the matrix m with random numbers in the range (start, end) */
